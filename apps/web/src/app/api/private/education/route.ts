...
export async function POST(request: Request) {
  try {
    ...
    if (!education) {
      return new NextResponse("Please provide education data", { status: 400 });
    }
    if (
      educationInputSchema.safeParse(educationData).success === false ||
      (skills?.length && skills.length > 5)
    ) {
      return new NextResponse("Invalid education data", { status: 400 });
    }
    const count = await prisma.education.count({
      where: { userId: session.user.id },
    });
    if (count > 5) {
      return new NextResponse("Education limit reached", { status: 403 });
    }
    ...
    return new NextResponse("Education added successfully", { status: 201 });
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (!id) {
      return new NextResponse("Education ID is required for update", {
        status: 400,
      });
    }
    ...
    return new NextResponse("Education updated successfully", { status: 200 });
  } catch (error) {
...
export async function DELETE(request: Request) {
  try {
    ...
    if (!id) {
      return new NextResponse("Education ID is required for deletion", {
        status: 400,
      });
    }
    ...
    return new NextResponse("Education deleted successfully", { status: 200 });
  } catch (error) {
